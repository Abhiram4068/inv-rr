...
const DeletedUsers = () => {
    ...
    const fetchDeletedUsers = async (page, search) => {
        try {
            setLoading(true);
            const data = await userService.getDeletedUsers({ page, search });
            setDeletedUsers(data.results || []);
    ...
    const handleConfirmAction = async () => {
        ...
        try {
            if (type === 'restore') {
                ...
                setDeletedUsers(prev => prev.filter(user => user.id !== targetUser.id));
        ...
        <div className="flex-1 bg-[#f0f2f7] min-h-screen p-8 font-sans relative">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
                    <div>
                        <h1 className="text-xl font-bold text-slate-800 tracking-tight">Deleted Accounts</h1>
                        <p className="text-xs text-slate-400 mt-0.5">Review deleted system profiles, restore operational accounts, or view past records.</p>
                    </div>
                    ...
                    <div className="relative w-full md:w-80">
                        ...
                        <input
                            type="text"
                            placeholder="Search deleted name, email, or role..."
                            value={searchTerm}
                ...
                <div className="bg-white rounded-sm overflow-hidden shadow-sm">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-slate-200 bg-slate-50 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                                    <th className="py-4 px-6">Deleted Personnel</th>
                                ...
                                {loading ? (
                                            ...
                                            <td className="py-4 px-6">
                                                <div className="flex flex-col">
                                                    <span className="font-semibold text-slate-800 flex items-center gap-2">
                                                        ...
                                                        <span className="w-1.5 h-1.5 rounded-full bg-slate-400 block" title="Deleted Account Status" />
                                                    </span>
                                    ...
                                    <tr>
                                        <td colSpan="5" className="text-center py-12 text-slate-400 font-medium">
                                            <i className="fa-solid fa-user-minus text-3xl mb-3 block text-slate-200"></i>
                                            No accounts are currently in the deleted state.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                    ...
                    <div className="bg-slate-50 border-t border-slate-200 px-6 py-3 flex flex-col sm:flex-row justify-between items-center gap-3">
                        <span className="text-xs text-slate-400 font-semibold uppercase tracking-tight">
                            Deleted: {totalCount} records identified
                        </span>
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400 leading-relaxed mt-2">
                                        Are you sure you want to restore <strong>{modalConfig.targetUser?.first_name} {modalConfig.targetUser?.last_name}</strong>? This action restores standard system interaction paths and brings the account back to an active state.
                                    </p>
                                </div>
                            </div>
                        </div>
